...
> - **나눔의 열매(`/community`) 폐기 → 온라인교육과정(`/curriculum`)으로 대체.** §6 P-04, §2.3 GNB가 이 문서에서도 이미 대체된 내용으로 수정되어 있습니다. 원본 나눔의 열매 명세는 §13.1에 보존해 두었습니다.
...
헤더 GNB는 **4개 항목 고정**. 순서 변경 금지.

| 순서 | 라벨 | 경로 | 영문 캡션(내부용) |
|---|---|---|---|
| 1 | 축복의 씨앗 | `/guide` | Blessing Guide |
| 2 | 행복의 꽃 | `/stories` | Our Stories |
| 3 | 온라인교육과정 | `/curriculum` | Online Courses |
...
- 로고 `alt` 텍스트: `블레싱월드 마크`
- 모바일(<768px): 햄버거 메뉴 → 드로어(Sheet)로 4개 항목 세로 나열
...
```
[로고마크][블레싱월드]                    축복의 씨앗  행복의 꽃  온라인교육과정  지역가정교회
```
...
| `/curriculum` | 온라인교육과정 — 축복교육 4강좌 | 축복결혼이 궁금한 분들을 위한 4개의 강좌를 순서대로, 또는 골라서 들어보세요. |
...
| 2 | 온라인교육과정 | 축복교육 4강좌 | 축복결혼이 궁금한 분들을 위한 4개의 강좌. 순서대로, 또는 궁금한 것부터 들어보세요. | `/curriculum` | `강좌 보기 →` |
...
### P-04. 온라인교육과정 `/curriculum` (실제 구현본 — 원본 나눔의 열매를 대체)

> 원본 사이트의 "나눔의 열매"(`/community`, 지역 나눔장터) 자리는 실제 구현에서 **온라인교육과정**으로
...
### 8.2 교육과정 관리 (`/admin/curriculum`) ✅ 구현됨
...
- 저장 동작은 8.2 교육과정 관리와 동일한 패턴(`src/lib/churches.ts`).
...
| ID | 기준 |
|---|---|
| AC-01 | GNB 4개 항목이 순서대로 표시되고, 현재 라우트가 시각적으로 강조된다 |
...
| **M5 — 온라인교육과정·지역가정교회 ✅** | P-04 온라인교육과정(강좌 목록·진행 표시), P-05 지역가정교회 디렉터리 — 원안의 나눔의 열매(§13.1)·가정민원실(§13.5)은 폐기 | 교육과정·지역가정교회 오픈 |
| **M6 — 운영 (부분 완료 ✅)** | 관리자 로그인(§8.1), 교육과정 관리(§8.2), 지역가정교회 관리(§8.3) 완료. 신청/스토리/FAQ/식구인증/통계 관리, 개인정보 자동 파기(§7.4)는 미착수 | 교육과정·지역가정교회 운영 이관 가능 |
| **M7 — 품질** | 접근성·성능·SEO 점검(§9), 개선 항목(§10) 반영 | 정식 오픈 |

> 실제 구현은 위 순서를 그대로 따르지 않고 M5/M6 중 온라인교육과정·지역가정교회·로그인 부분을
