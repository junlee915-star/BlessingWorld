// 대한민국 시·도 / 시·군·구 목록 — `/onboarding` §P-07 Step 3(지역) 2단 연동 Select에서 씁니다.
// §src/content/churches.ts의 DEFAULT_CHURCHES와 달리 이 목록은 "실제 등록된 지역교회가
// 있는 곳"이 아니라 "방문자가 어디 사는지"를 고르기 위한 것이라, 행정구역 전체를 담습니다.
// 상세 주소는 수집하지 않으므로(§P-07 "시·군·구까지만 수집") 시·군·구 이름까지만 있습니다.
//
// 세종특별자치시는 하위 구가 없어 시·군·구 목록이 자기 자신(세종시) 하나뿐입니다.
...
export const REGIONS: RegionGroup[] = [
  ...
  { sido: "광주", sigungu: ["동구", "서구", "남구", "북구", "광산구"] },
  { sido: "대전", sigungu: ["동구", "중구", "서구", "유성구", "대덕구"] },
  { sido: "울산", sigungu: ["중구", "남구", "동구", "북구", "울주군"] },
  { sido: "세종", sigungu: ["세종시"] },
  {
    sido: "경기",
    sigungu: [
...
export function getSigunguOptions(sido: string): string[] {
  return REGIONS.find((group) => group.sido === sido)?.sigungu ?? [];
}
